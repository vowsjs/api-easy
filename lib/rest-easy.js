...
exports.describe = function (text) {
  return {
    ...
    //
    // ### Configure Headers
    // Manipulate the headers that are sent to your API using these methods. 
    // They are designed to mimic the node.js core APIs to some level.
    //
    ...
    // Control the base path used by a given suite. Append a path
    // by calling `.path()`. Remove the last `num` paths from the 
    // suite by calling `.unpath(num)`. Set the root path using `.root(path)` 
    //
    path: function (uri) {
    ...
    //
    // ### Perform HTTP Requests in Tests
    // The `.get()`, `.post()`, `.put()`, `.del()`, and `.head()` 
    // methods add a new topic to the vows structure maintained by this
    // RESTeasy suite. The nuts and bolts of this are in the "private"
    // method `_request()`.
    //
    // Each method invocation returns the suite itself so that 
    //`.expect()` and other setup methods can be called afterwards.
    //
    ...
    // sequential tests. This is precisely what `.next()` does. It will take
    // add the object representing the current batch (or 'vows') and add it to the 
    // suite and reset the context for the `this.current` test. 
    //
    next: function (reset) {
    ...
    _request: function (/* method [uri, data, params] */) {
      ...
      options.uri = 'http://' + this.host + port + fullUri;
